...
export class TripDocumentService {
	 ...
	 *
	 */
	async uploadDocument(data: UploadDocumentData): Promise<TripDocumentWithUrl> {
		...
		return {
			...document,
			isImage: this.isImage(document.name),
			publicUrl: this.getPublicUrl(document.file_path),
		};
	}
	 ...
	 *
	 */
	async getTripDocumentsWithUrl(
		tripId: string,
	): Promise<TripDocumentWithUrl[]> {
		const documents = await this.getTripDocuments(tripId);

		return documents.map((doc) => ({
			...doc,
			isImage: this.isImage(doc.name),
	...
	async deleteDocument(documentId: string): Promise<void> {
		...
		if (storageError) {
			...
			throw new Error(
				`Error al eliminar archivo del storage: ${storageError.message}`,
			);
		}
	...
	private async verifyMembership(
		...
		if (error || !data) {
			throw new Error(
				"Solo los miembros del viaje pueden realizar esta acción",
			);
		}
	}
}
