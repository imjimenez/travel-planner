...
@Component({
  ...
  <form [formGroup]="registerForm" (ngSubmit)="onSubmit()" class="space-y-4">
    ...
    <div class="space-y-2">
      ...
      <input pInputText id="fullName" type="text" formControlName="fullName" placeholder="Tu nombre completo" required
      ...
      @if (registerForm.controls.fullName.errors && registerForm.controls.fullName.touched) {
        @if(registerForm.controls.fullName.errors['required']) {
          <p-message severity="error" size="small" variant="simple">Nombre completo es requerido</p-message>
        }
      }
    </div>
    ...
    <div class="space-y-2">
      ...
      <input pInputText id="email" type="email" formControlName="email" placeholder="tu@email.com" required
        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all outline-none"/>
      @if (registerForm.controls.email.errors && registerForm.controls.email.touched) {
        @if(registerForm.controls.email.errors['required']) {
          <p-message severity="error" size="small" variant="simple">Correo electrónico es requerido</p-message>
        }@else{
          <p-message severity="error" size="small" variant="simple">Correo electrónico no válido</p-message>
        }
      }
    </div>
    ...
    <div class="space-y-2">
      ...
      <p-inputgroup class="relative">
        <input pInputText id="password" [type]="showPassword ? 'text' : 'password'" formControlName="password" placeholder="Tu contraseña" required
    ...
    <div class="flex items-start gap-3">
      ...
      @if (registerForm.controls.termsAndPrivacy.errors && registerForm.controls.termsAndPrivacy.touched) {
        @if(registerForm.controls.termsAndPrivacy.errors['required']) {
          <p-message severity="error" size="small" variant="simple">Debe aceptar los términos y la política de privacidad</p-message>
        }
      }
    </div>
    <!-- Botón de submit -->
    <p-button type="submit" [disabled]="registerForm.invalid || loading()" styleClass="w-full space-y-2 py-3 rounded-lg font-medium transform hover:scale-[1.02] transition-all">
        @if(loading()){
...
export default class RegisterForm {
	...
	registerForm = this.#fb.group({
		...
		termsAndPrivacy: this.#fb.nonNullable.control(false, [
