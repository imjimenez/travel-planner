...
@Component({
    ...
    <form [formGroup]="loginForm" class="space-y-4" (ngSubmit)="onSubmit()">
      ...
      <div class="text-right">
        <a routerLink="/auth/forgot-password" class="text-sm text-blue-600 hover:text-blue-700 font-medium transition-colors">
          ¿Olvidaste tu contraseña?
        </a>
      </div>

      <!-- Submit -->
      <p-button type="submit" [disabled]="loginForm.invalid || loading()" styleClass="w-full  py-3 rounded-lg font-medium transform hover:scale-[1.02] transition-all">
          @if(loading()){
