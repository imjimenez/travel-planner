...
@Component({
  ...
  template: `
    ...
    <div
      class="flex-1 overflow-y-auto lg:p-6"
      ...
      <div class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <div class="group flex items-center gap-3 p-3 bg-gray-50 rounded-lg transition-colors">
              ...
              <div class="flex-1 min-w-0">
                ...
                <!-- MODO VISTA -->
                <div class="flex flex-col lg:flex-row items-start gap-2 lg:gap-6">
                  <!-- Título -->
                ...
                <!-- Detalles -->
                @if (todo.details) {
                <p class="text-xs text-gray-500 mt-1">{{ todo.details }}</p>
                } }
              </div>

              <!-- Botones de acción -->
              <div
                class="hidden lg:flex shrink-0 items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
              >
                @if (editingTask() === todo.id) {
              ...
              <!-- Botones de acción -->
              <div class="lg:hidden shrink-0 flex items-center gap-1 transition-opacity">
                @if (editingTask() === todo.id) {
                <!-- Botones de edición: Guardar y Cancelar -->
                <button
                  type="button"
        ...
        <div>
          ...
          <div class="space-y-2">
            ...
            <div class="group flex items-center gap-3 p-3 bg-green-50 rounded-lg transition-colors">
              <!-- Checkbox -->
              <button
                type="button"
                (click)="toggleTodo(todo)"
              ...
              <div class="flex-1 min-w-0">
                ...
                <!-- Detalles -->
                @if (todo.details) {
                <p class="text-xs text-gray-400 mt-1 line-through">{{ todo.details }}</p>
                }
              </div>

              <!-- Botón eliminar -->
              <div
                class="hidden lg:flex shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
              >
                <button
                  type="button"
                  (click)="deleteTodo($event, todo)"
              ...
              <div class="flex lg:hidden shrink-0 transition-opacity">
                <button
                  type="button"
                  (click)="deleteTodo($event, todo)"
                  class="w-7 h-7 flex items-center justify-center text-red-600 hover:bg-red-100 rounded transition-colors cursor-pointer"
...
export class ChecklistModalComponent {
   ...
   */
  pendingTodos = computed(() => this.todos().filter((t) => t.status !== 'completed'));

  /**
   * Computed: Tareas completadas
   */
  completedTodos = computed(() => this.todos().filter((t) => t.status === 'completed'));
